...
export class PayrolldetailsComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    this.globalServ.getMyCompLabels('payrolDetails');
    this.getProjectList();
    this.getDepartments();
  ...
  saveExcel() {
    ...
    this.httpGetService
      .getExcel(
        'reports/payrollInfo/xls?empCode=' +
        this.reportObj.employeeCode +
        '&deptCode=' +
        this.reportObj.department +
        '&payrollCode=' +
      ...
      .subscribe((res: any) => {
        ...
        FileSaver.saveAs(
          data,
          'Payroll_Details' + new Date().getTime() + EXCEL_EXTENSION
        );
  ...
  savePDF() {
    ...
    this.spinner.show();
    this.httpGetService
      .getPdf('reports/payrollInfo/pdf?empCode=' + this.reportObj.employeeCode + '&payrollCode=' + this.reportObj.payrollCode + '&deptCode=' + this.reportObj.department +
        '&year=' + this.year + '&date=' + this.date + '&month=' + this.month
    ).subscribe((res: any) => {
        this.spinner.hide();
        const file = new Blob([res], { type: 'application/pdf' });
      FileSaver.saveAs(file, 'Payroll_Details' + new Date().getTime() + '.pdf');
